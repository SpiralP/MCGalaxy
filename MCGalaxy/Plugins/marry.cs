...
namespace Core {
	...
	public sealed class CmdDivorce : Command {
		...
		public override void Use(Player p, string message) {
			string marriedTo = MarryPlugin.marriages.Get(p.name);
	...
	public sealed class CmdMarry : Command {
		...
		public override void Use(Player p, string message) {
			string entry = MarryPlugin.marriages.Get(p.name);
			if (entry != null) {
			...
			entry = MarryPlugin.marriages.Get(partner.name);
			if (entry != null) {
